...
- **Annotation callouts (`scene3d.Annotation` + `scene3d.annotate`).** A named text
  ...
  right next to it.
- **Per-vertex weighted transform references** (`Mesh` `transform_refs` +
  `transform_indices` + `transform_weights`): vertices positioned by a weighted blend of
  named `Group` transforms (a bending tube, a fault drag zone) — driving a referenced
  Group's transform via `$state`/`Plot.channel` animates the deformation with no geometry
  re-upload, and picking sees the deformed surface, so `pick-at` reports deformed positions.
