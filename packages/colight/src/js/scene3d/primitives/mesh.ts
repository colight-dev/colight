...
import { BaseComponentConfig, GeometryData, VertexBufferLayout } from "../types";
import { definePrimitive, attr, createVertexBufferLayout } from "./define";
import {
  cameraStruct,
  lightingConstants,
  lightingCalc,
...
function detectFormat(geometry: StructuredGeometry): VertexFormat {
  ...
  if (hasColors) {
    ...
    const posCount = Array.isArray(positions) ? positions.length / 3 : positions.length / 3;
    const colorLen = Array.isArray(geometry.colors) ? geometry.colors.length : geometry.colors!.length;
...
function computeFlatNormals(
  ...
  const pos = Array.isArray(positions) ? new Float32Array(positions) : positions;
  ...
  if (!indices) {
    ...
    for (let i = 0; i < idx.length; i += 3) {
      const v0 = idx[i], v1 = idx[i + 1], v2 = idx[i + 2];
 ...
 */
export function interleaveVertexData(geometry: StructuredGeometry): ProcessedMeshGeometry {
  ...
  const colors = geometry.colors
    ? (Array.isArray(geometry.colors) ? new Float32Array(geometry.colors) : geometry.colors)
    : undefined;

  const uvs = geometry.uvs
    ? (Array.isArray(geometry.uvs) ? new Float32Array(geometry.uvs) : geometry.uvs)
    : undefined;
 ...
 */
export function getFormatKey(format: VertexFormat, shading: "lit" | "unlit", hasTexture = false): string {
...
function buildGeometryLayout(format: VertexFormat): VertexBufferLayout {
  ...
  // Color (optional)
  if (format.hasColors) {
    const colorFormat: GPUVertexFormat = format.colorComponents === 4 ? "float32x4" : "float32x3";
    attributes.push({ shaderLocation: location++, offset, format: colorFormat });
    offset += format.colorComponents * 4;
  }

  // UV (optional)
  if (format.hasUVs) {
    attributes.push({ shaderLocation: location++, offset, format: "float32x2" });
    offset += 8;
  }
 ...
 */
function generateMeshVertexShader(format: VertexFormat, forPicking: boolean, hasTexture = false): string {
  // Build vertex inputs from geometry buffer
  const geoInputs: string[] = ["@location(0) localPos: vec3<f32>"];
  let geoLoc = 1;
  if (format.hasNormals) geoInputs.push(`@location(${geoLoc++}) normal: vec3<f32>`);
  if (format.hasColors) {
  ...
  if (!forPicking) {
    if (format.hasColors) {
      ...
      colorComputation = format.colorComponents === 4
      ...
      colorComputation = "let finalColor = instanceColor;\n  let finalAlpha = alpha;";
    }
  }
 ...
 */
function generateMeshFragmentShader(shading: "lit" | "unlit", hasTexture = false): string {
...
export function defineMesh(
  ...
  const getProcessed = typeof geometry === "function"
    ? () => {
        const geo = geometry();
  ...
  const hasNormals = shading === "lit" || (sampleGeo.normals !== undefined && sampleGeo.normals.length > 0);
  const format: VertexFormat = {
    hasNormals,
    hasColors: sampleGeo.colors !== undefined && sampleGeo.colors.length > 0,
    colorComponents: sampleGeo.colors
      ? ((Array.isArray(sampleGeo.colors) ? sampleGeo.colors.length : sampleGeo.colors.length) /
         ((Array.isArray(sampleGeo.positions) ? sampleGeo.positions.length : sampleGeo.positions.length) / 3) === 4 ? 4 : 3)
      : 3,
  ...
  const pickingInstanceLayout = buildPickingInstanceLayout(instanceStartLocation);
