...
import { probeStage, probeStateChange, PROBE_STAGES } from "./probe";
...
export async function createStateStore(data) {
  ...
  const applyUpdates = (updates) => {
    ...
    if (isRoot) {
      transactionUpdates = [];
      // Probe: open the state-change -> render-submitted span. Nested updates
      // belong to the same span, so only the root opens it.
      probeStateChange();
    }
  ...
  const $state = new Proxy(
    {
      // The one non-recursive entry into eval.js's `evaluate` (which recurses
      // per AST node), so this is where the probe measures AST re-evaluation.
      evaluate: (ast) =>
        probeStage(PROBE_STAGES.evaluate, () =>
          evaluate(ast, $state, experimental, buffers),
        ),
      whenReady(settleWindowMs) {
        return readyState.whenReady(settleWindowMs);
      },
      beginUpdate(label) {
