...
This specification describes the live sync behavior between the colight server and client. The implementation is located in:

- **Client**: `packages/colight/src/js/live/live.jsx` (main app), `websocket-message-handler.js` (message processing), `TopBar.jsx` (UI components)
- **Server**: `packages/colight/src/colight/server/server.py` (WebSocket server and file watching)
