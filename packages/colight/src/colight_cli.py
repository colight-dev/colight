...
def live(
    ...
    server = LiveServer(
        ...
        host=host,
        http_port=port,
        # WebSocket port is HTTP port + 1; with port 0 the server picks a
        # free adjacent pair itself.
        ws_port=port + 1 if port else 0,
        open_url=not no_open,
        open_path=open_path,
    )
...
def inspect(target: pathlib.Path, as_json: bool):
    ...
    VISUAL = {"components": [{"path", "count", "instances"}],
              ...
              "channels"?: [{"component", "parameter", "rule", "prop",
                             "domain"?, "samples"?}],
              "declarations"?: [{"path", "label", "filter_by"?, "color_by"?,
    ...
    ``channels`` reports what is sweepable without Python: each entry names the
    ``$state`` parameter, the prop it drives, the sample domain and the
    resampling rule (components built with ``Plot.channel``).
    ``declarations`` regroups those same facts per component, keyed by the
    walker's stable unique ``path`` — the identical section
    ``colight screenshot --json`` reports next to its pixel-derived coverage.
    """
    ...
    def echo_visual(visual: Dict[str, Any], warnings: List[Dict[str, str]]) -> None:
        ...
        for chan in visual.get("channels", []):
            parts = [f"channel {chan['parameter']!r}"]
            if chan.get("domain") is not None:
                parts.append(f"domain {chan['domain']}")
            if chan.get("samples") is not None:
                parts.append(f"{chan['samples']} samples")
            parts.append(f"rule {chan['rule']}")
            if chan.get("prop"):
                parts.append(f"drives {chan['prop']}")
            click.echo(f"  {' '.join(parts)} on {chan['component']}")
...
@click.option(
    ...
    default=10.0,
    show_default=True,
    help="Max seconds to wait for the scene to settle before capturing it "
    'anyway and reporting "settled": false (0 to wait for settling only, up '
    "to --ready-timeout).",
)
@click.option(
    "--probe-param",
    type=str,
    help="Frame-time probe: sweep this $state key and report per-stage CPU "
    "timings (AST evaluate, compileScene, the equality gate, render) under "
    'the JSON "probe" key. Implies direct rendering.',
)
@click.option(
    "--probe-range",
    type=str,
    default="0,1",
    show_default=True,
    help='Sweep range "lo,hi" for --probe-param.',
)
@click.option(
    "--probe-frames",
    type=int,
    default=60,
    show_default=True,
    help="Number of swept frames to measure.",
)
@click.option(
    "--probe-warmup",
    type=int,
    default=2,
    show_default=True,
    help="Frames rendered and discarded before measuring (first-frame "
    "pipeline creation would otherwise dominate).",
)
@click.option(
    "--no-daemon",
    is_flag=True,
...
def screenshot(
    ...
    Frame-time probe: --probe-param KEY sweeps a $state key across
    --probe-range lo,hi in --probe-frames steps, driving the same update
    path a slider drives and waiting for each render, then reports where
    the per-state-change CPU actually goes — AST re-evaluation,
    compileScene, each walk of the render effect's equality gate, the
    render itself, and the end-to-end span — plus writeBuffer calls/bytes
    per frame and rAF-to-rAF frame intervals against the 60/120fps
    budgets. The capture is the scene at the sweep's final value. Probing
    always renders directly (a shared daemon tab would mix other
    requests' frames into the samples) and is single-view only.
    ...
    JSON schema:
      {"target": str, "out": str, "width": int, "height": int, "dpr": float,
       ...
       "declarations"?: [{"path": str, "label": str, "component"?: int,
                          "filter_by"?: {"label"?, "min", "max"},
                          "color_by"?: {"label"?, "cmap", "domain"?,
                                        "categorical", "categories"?},
                          "color_channels"?: {"active"?: str,
                            "channels": [{"name", "label", "kind"}]},
                          "channels"?: [{"parameter", "rule", "prop",
                                         "domain"?, "samples"?}]}],
       "probe"?: {"param": str, "range": [lo, hi], "frames": int,
                  "warmup": int, "measured_frames": int,
                  "stages": {"<stage>": {"count", "occurrences",
                    "median_ms", "p95_ms", "max_ms", "mean_ms",
                    "total_ms"}},
                  "writes": {"frames", "calls_median", "calls_total",
                             "bytes_median", "bytes_p95", "bytes_total"},
                  "frame": {"count", "interval_median_ms",
                            "interval_p95_ms", "interval_max_ms",
                            "fps_median", "budget_60fps_ms",
                            "budget_120fps_ms"}}}
    ...
    in the capture; declarations report what the visual DECLARES — the
    filters, encodings and sweepable `Plot.channel` parameters — from the
    same payload walk `colight inspect` uses, keyed by `path`, with
    `component` present only where the mapping onto coverage's compiled
    indices is unambiguous.)
    """
    from colight.cli_tools import (
        daemon_client,
        probe_tools,
        scene_pick,
        screenshot_tools,
    ...
    effective_timeout = None if ready_timeout <= 0 else ready_timeout
    # 0 means "no fallback": wait for the scene to settle for as long as
    # --ready-timeout allows, which is the pre-two-tier behaviour.
    effective_settle_window = None if settle_window <= 0 else settle_window
    ...
    def run_shot(
        ...
        run_probe: Optional[dict] = None,
    ) -> dict:
        payload = None
        # A probe run needs a cold tab it owns for the whole sweep, so it
        # never goes through the daemon's shared warm scenes.
        if not no_daemon and run_probe is None:
            payload = daemon_client.try_screenshot(
                ...
                rulers=run_rulers,
                views=run_views,
                settle_window=effective_settle_window,
            )
        if payload is None:
    ...
    try:
        ...
        if probe_param:
            ...
            if probe_warmup < 0:
                raise ValueError("--probe-warmup must be >= 0")
            lo, hi = probe_tools.parse_range(probe_range)
            probe_spec = {
                "param": probe_param,
                "lo": lo,
                "hi": hi,
        ...
        payload = run_shot(
            width, height, out, check, frame, rulers, view_names, probe_spec
        )
        if max_edge is not None:
    ...
    if as_json:
        ...
        if "probe" in payload:
            for line in probe_tools.format_summary(payload["probe"]):
                click.echo(line)
