...
def _query_disc(
...
) -> Optional[Tuple[slice, slice, np.ndarray, np.ndarray]]:
    """Pixels sampled by a radius disc around a page CSS point.

    A pixel is in the disc when its center is within ``radius`` of the query
    point — except at radius 0, where no pixel center generally coincides with
    the query point (a pixel's center sits at its integer index + 0.5, so a
    whole-number page coordinate is always 0.707 device px from the nearest
    center). Radius 0 therefore means "the single pixel containing the query
    point", which is what a point query should sample: it is the same pixel
    ``pick-where`` reports and the one under the cursor on screen.

    Args:
        snapshot: A full-frame snapshot.
        x: Page CSS x (origin top-left).
        y: Page CSS y (y down).
        radius: Disc radius in CSS pixels (0 = the containing pixel).

    Returns:
        ``(rows, cols, mask, distances)`` where ``rows``/``cols`` index the
        snapshot, ``mask`` marks in-disc pixels and ``distances`` holds device
        pixel distances from the query point. None when the disc lies entirely
        off-canvas or samples no pixel.
    """
    cx, cy = _page_to_device(snapshot, x, y)
    device_radius = radius * snapshot.dpr

    if device_radius <= 0:
        # Point query: the pixel whose extent contains (cx, cy).
        col = int(math.floor(cx))
        row = int(math.floor(cy))
        if not (0 <= col < snapshot.width and 0 <= row < snapshot.height):
            return None
        rows, cols = slice(row, row + 1), slice(col, col + 1)
        distances = np.hypot(np.array([[col + 0.5 - cx]]), np.array([[row + 0.5 - cy]]))
        return rows, cols, np.ones((1, 1), dtype=bool), distances
    ...
    if not disc.any():
        return None
    return slice(y0, y1 + 1), slice(x0, x1 + 1), disc, distances
...
def hits_at(
    ...
    Args:
        ...
        x: Page CSS x (origin top-left).
        y: Page CSS y (y down).
        radius: Disc radius in CSS pixels. 0 samples the single pixel
    ...
    """
    sampled = _query_disc(snapshot, x, y, radius)
    if sampled is None:
        return []
    rows, cols, disc, distances = sampled
    disc_pixels = int(disc.sum())
    ids = snapshot.ids[rows, cols]
...
def pick_at_source(
    ...
    with source.scene() as scene:
        ...
        # Background share over the same pixels hits_at sampled.
        background_share = 1.0
        sampled = _query_disc(snapshot, x, y, radius)
        if sampled is not None:
            rows, cols, disc, _ = sampled
            ids = snapshot.ids[rows, cols]
            background_share = round(float(((ids == 0) & disc).sum() / disc.sum()), 4)
