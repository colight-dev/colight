...
class RenderSessionLike(Protocol):
    ...
    def settle_status(self) -> Tuple[bool, Optional[str]]:
        """Whether the last render settled, and why not if it didn't.

        Optional: sessions that cannot report it are treated as settled
        (see :func:`_session_settle_status`).
        """
        ...
...
def _session_settle_status(session: RenderSessionLike) -> Tuple[bool, Optional[str]]:
    """Settle status of a session's last render, defaulting to settled.

    A session that cannot report settling (an older remote session) is
    treated as settled: verify's behaviour is then exactly what it was.
    """
    getter = getattr(session, "settle_status", None)
    if getter is None:
        return True, None
    return getter()
...
def _render_screenshot(
    ...
    Carries ``settled``/``settle_reason`` when the scene was still animating
    at capture time, so callers can refuse to trust the hash.
    """
    ...
    settled, settle_reason = _session_settle_status(session)
    if not settled:
        shot["settled"] = False
        if settle_reason:
            shot["settle_reason"] = settle_reason
    return shot
    ...
    with factory(width, dpr) as session:
        for visual, golden in pairs:
            ...
            if check_pixels and golden_shot:
                ...
                if shot.get("settled") is False:
                    # The comparison ran, and its result is reported as-is,
                    # but the hash came from a scene that was still moving:
                    # neither a match nor a mismatch means much here. Warn
                    # loudly rather than silently trusting or failing it.
                    entry["pixels"]["settled"] = False
                    if shot.get("settle_reason"):
                        entry["pixels"]["settle_reason"] = shot["settle_reason"]
                    warnings.append(
                        f"{visual['id']}: pixel hash captured while the scene was "
                        "still animating (settled=false) — this comparison is "
...
def update_target(
    ...
    # A golden must be settled. Recording a hash captured mid-animation would
    # bake a non-reproducible value into the baseline, so every later verify
    # of that block fails for no real reason. Refuse the whole update rather
    # than writing a mix of trustworthy and untrustworthy goldens.
    unsettled = [
        entry
        for entry in entries
        if (entry.get("screenshot") or {}).get("settled") is False
    ]
    if unsettled:
        details = []
        for entry in unsettled:
            reason = (entry.get("screenshot") or {}).get("settle_reason")
            details.append(f"{entry['id']}" + (f" ({reason})" if reason else ""))
        payload["status"] = "error"
        payload["unsettled"] = [entry["id"] for entry in unsettled]
        payload["errors"] = [
            {
                "error": {
                    "type": "UnsettledGoldenError",
                    "message": (
                        "refusing to record a golden from a scene that never "
                        "settled: " + ", ".join(details) + ". A golden must be "
                        "a deterministic capture; make the visual settle (or "
                        "record it with --no-pixels) and try again."
                    ),
                }
            }
        ]
