...
def try_screenshot(
    ...
    settle_window: Optional[float] = None,
...
class RemoteRenderSession:
    ...
    def render(
        ...
        self._settled = payload.get("settled", True) is not False
        self._settle_reason = (
            payload.get("settle_reason") if not self._settled else None
        )
        return (
    ...
    def settle_status(self) -> Tuple[bool, Optional[str]]:
        """Settle outcome the daemon reported for the last render."""
        return self._settled, self._settle_reason
