...
from . import compose, probe_tools, targets

DEFAULT_WIDTH = 800
DEFAULT_DPR = 1.0
DEFAULT_READY_TIMEOUT = 30.0

# How long to wait for a scene to settle before capturing it anyway and
# labelling the result unsettled. Deliberately well under
# DEFAULT_READY_TIMEOUT so a continuously-animating scene is captured and
# labelled rather than dying at the hard timeout; still generous enough that
# a heavy scene which *does* settle takes the quiescence path unchanged.
DEFAULT_SETTLE_WINDOW = 10.0
...
class SceneLike(Protocol):
    ...
    def visual(self) -> Optional[Tuple[Dict[str, Any], List[bytes]]]:
        """The (data, buffers) this scene was loaded from, or None.

        The declarative facts a visual carries (``filter_by``, ``color_by``,
        ``color_channels``, ``Plot.channel`` parameters) live in the payload,
        not the pixels. Exposing the loaded payload here lets
        ``screenshot --json`` report them from the same structure walk
        ``colight inspect`` uses, in both direct and daemon modes."""
        ...

    def capture(self) -> Tuple[bytes, int, int]: ...

    def settle_status(self) -> Tuple[bool, Optional[str]]:
        """Whether the scene settled before capture, and why not if it didn't.

        ``(True, None)`` is the normal case: all pending render work drained,
        so the capture is deterministic. ``(False, reason)`` means the settle
        window elapsed on a scene that was still animating — the pixels are
        real, but they are a snapshot of a moving target.
        """
        ...
...
class RenderSession:
    ...
    def __init__(
        ...
        settle_window: Optional[float] = DEFAULT_SETTLE_WINDOW,
    ) -> None:
        self._width = width
        self._dpr = dpr
        self._debug = debug
        ...
        self._visual: Optional[Tuple[Dict[str, Any], List[bytes]]] = None
    ...
    def load(
        ...
        if self._studio is None:
            self._studio = StudioContext(
                ...
                settle_window=self._settle_window,
                reuse=True,
                keep_alive=1.0,
    ...
    def visual(self) -> Optional[Tuple[Dict[str, Any], List[bytes]]]:
        """The (data, buffers) most recently loaded, or None before ``load``."""
        return self._visual

    def settle_status(self) -> Tuple[bool, Optional[str]]:
        """Settle outcome of the most recent load (see :class:`SceneLike`)."""
        studio = self.studio
        return studio.settled, studio.settle_reason
...
def render_png(
    ...
    Args:
        ...
        settle_window: Seconds to wait for quiescence before capturing an
            animating scene anyway (see :data:`DEFAULT_SETTLE_WINDOW`).
...
def collect_declarations(
    scene: SceneLike, coverage: Optional[Dict[str, Any]] = None
) -> List[Dict[str, Any]]:
    """Declarative facts of the visual being captured (payload-derived).

    Coverage answers "what is on screen"; this answers "what does this visual
    declare" — the `filter_by` thresholds, `color_by` encodings, switchable
    `color_channels` roster and `Plot.channel` parameters an agent can sweep.
    They live in the payload, so they are read from the SAME structure walk
    ``colight inspect`` uses (:func:`inspect_tools.declarations_payload`)
    rather than from DOM markers — one source of truth, and identical output
    in direct and daemon modes.

    Returns:
        Declaration entries (empty when the visual declares nothing, or when
        the scene source cannot supply its payload).
    """
    ...
    if loaded is None:
        return []
    data, buffers = loaded
    return inspect_tools.declarations_payload(data, buffers, coverage)
...
def _capture_scene(
    ...
    declarations = collect_declarations(scene, extras.get("coverage"))
    if declarations:
        extras["declarations"] = declarations

    # Two-tier readiness: a settled capture is deterministic; an unsettled one
    # is a snapshot of a scene that was still animating. Report the flag only
    # when it is false, so the common (settled) payload is unchanged and the
    # key's presence is itself the signal.
    settled, settle_reason = scene.settle_status()
    if not settled:
        extras["settled"] = False
        if settle_reason:
            extras["settle_reason"] = settle_reason
        extras.setdefault("warnings", []).append(
            "captured while still animating: the scene did not settle, so "
            "these pixels are not reproducible (settled=false)"
        )
...
def screenshot_source(
    ...
    Args:
        ...
        probe: Frame-time probe spec ``{"param", "lo", "hi", "frames",
            "warmup"}``. When given, the parameter is swept before capture and
            the resulting per-stage timings land in the payload's ``probe``
            key; the captured PNG is the scene at the sweep's final value.
    ...
    if probe and views:
        raise ValueError(
            "--probe-param measures one scene's state path; a contact sheet "
            "re-renders the scene once per view, which would mix camera "
            "changes into the samples"
        )
    ...
    if views:
        with source.scene() as scene:
            ...
            legends = collect_dom_legends(scene.studio)
            declarations = collect_declarations(scene)
            settled, settle_reason = scene.settle_status()
        ...
        if not settled:
            ...
            if settle_reason:
                payload["settle_reason"] = settle_reason
    ...
    with source.scene() as scene:
        if probe:
            # Sweep before capture: the probe owns this tab for the duration,
            # and the capture that follows shows the sweep's final value.
            scene.mark_mutated()
            probe_payload = probe_tools.run_sweep(
                scene,
                probe["param"],
                probe["lo"],
                probe["hi"],
                probe["frames"],
                warmup=probe.get("warmup", 2),
            )
...
def screenshot_target(
    ...
    Args:
        ...
        probe: Frame-time probe spec ``{"param", "lo", "hi", "frames",
            "warmup"}``; sweeps the parameter and reports per-stage timings
            under the payload's ``probe`` key.
        settle_window: Seconds to wait for the scene to settle before
            capturing it anyway and reporting ``settled: false``.
    ...
    Returns:
        ...
        domain?, categorical, categories?}), ``declarations`` (payload-derived
        declarative facts per component: filter_by, color_by, color_channels,
        channels — the same facts ``colight inspect`` reports), and
        ``deterministic`` (only when
        ``check`` is set — compares the underlying renders;
        ``sha256_recheck`` is added when they differ). ``settled`` (false)
        plus ``settle_reason`` appear only when the scene was still
        animating at capture time; a settled capture omits both.
    """
