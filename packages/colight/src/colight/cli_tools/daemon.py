...
class _SceneHandle:
    ...
    def __init__(
        self,
        studio: StudioContext,
        entry: Optional[WarmScene] = None,
        data: Optional[Dict[str, Any]] = None,
        buffers: Optional[List[bytes]] = None,
    ) -> None:
        self.studio = studio
        self._entry = entry
        self._data = data if data is not None else (entry.data if entry else None)
        self._buffers = (
            buffers if buffers is not None else (entry.buffers if entry else None)
        )

    def visual(self) -> Optional[Tuple[Dict[str, Any], List[bytes]]]:
        """The (data, buffers) this scene was loaded from.

        Warm and fresh scenes both keep them, so the daemon reports the same
        payload-derived ``declarations`` as direct mode.
        """
        if self._data is None or self._buffers is None:
            return None
        return self._data, self._buffers
    ...
    def settle_status(self) -> Tuple[bool, Optional[str]]:
        """Settle outcome of the load that produced this scene.

        A warm scene carries the outcome of the load it was cached from,
        which is what the caller asked about: whether those pixels came
        from a settled scene.
        """
        return self.studio.settled, self.studio.settle_reason
...
class SceneService:
    ...
    def scene(
        ...
        fresh: bool = False,
        settle_window: Optional[float] = screenshot_tools.DEFAULT_SETTLE_WINDOW,
        ...
        try:
            ...
            if fresh:
                try:
                    yield _SceneHandle(studio, data=data, buffers=buffers)
                finally:
                    studio.stop()
                return
...
class _ServiceSceneSource:
    ...
    def scene(self, fresh: bool = False) -> Iterator[_SceneHandle]:
        ...
        with self._service.scene(
            ...
            fresh=fresh,
            settle_window=body.get(
                "settle_window", screenshot_tools.DEFAULT_SETTLE_WINDOW
            ),
        ) as handle:
            entry = handle._entry
            if entry is not None:
...
def _handle_render(server: "DaemonServer", body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        with screenshot_tools.RenderSession(
            ...
            settled, settle_reason = session.settle_status()
    finally:
