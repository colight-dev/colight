...
class TestBufferAnalysis:
    ...
    def test_hits_at_radius_zero_samples_the_containing_pixel(self):
        """radius 0 is a point query: it must read the pixel under the point.

        A pixel's center sits at its integer index + 0.5, so a whole-number
        page coordinate is 0.707 device px from the nearest center — a
        `distance <= 0` disc test selects nothing and every radius-0 query
        returned no hits, even on a surface the pick buffer clearly covers.
        """
        ids = np.zeros((20, 20), dtype=np.uint32)
        ids[10, 10] = 2  # component 0, instance 1
        for dpr in (1.0, 2.0):
            snapshot = make_snapshot(ids, dpr=dpr)
            hits = scene_pick.hits_at(snapshot, 10 / dpr, 10 / dpr, 0)
            assert [(h["component"], h["instance"]) for h in hits] == [(0, 1)], dpr
            # A known-background pixel still misses at radius 0.
            assert scene_pick.hits_at(snapshot, 2 / dpr, 2 / dpr, 0) == []

    def test_hits_at_radius_zero_respects_rect_offset(self):
        ids = np.zeros((20, 20), dtype=np.uint32)
        ids[10, 10] = 2
        snapshot = make_snapshot(ids, rect_left=100.0, rect_top=50.0)
        hits = scene_pick.hits_at(snapshot, 110.0, 60.0, 0)
        assert [(h["component"], h["instance"]) for h in hits] == [(0, 1)]
        # Off-canvas: no crash, no hits.
        assert scene_pick.hits_at(snapshot, 5000, 5000, 0) == []
