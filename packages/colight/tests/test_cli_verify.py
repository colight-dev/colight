...
class FakeSession:
    """A render session whose settle status the test controls.

    Renders fixed bytes per visual (so hashes are stable) and reports
    whatever settle status the test asks for -- the seam verify uses to
    decide whether a hash is trustworthy.
    """

    def __init__(self, settled: bool, png: bytes = b"fake-png") -> None:
    ...
    def render(self, data, buffers, height=None):
        return self._png, 400, 300
    ...
    def settle_status(self):
        ...
        return (
            False,
            "scene still animating after 50ms (2 render updates still pending)",
        )


class TestVerifySettling:
    """A golden must be settled; an unsettled comparison is not trustworthy."""

    def factory(self, settled: bool):
        return lambda width, dpr: FakeSession(settled)

    def test_update_refuses_unsettled_golden(self, project: pathlib.Path):
        """Recording a NEW golden from an unsettled capture is refused."""
        path = write_nb(project)
        result = verify_tools.update_target(
            path, session_factory=self.factory(settled=False)
        )
        assert result["status"] == "error"
        assert result["errors"][0]["error"]["type"] == "UnsettledGoldenError"
        assert "refusing to record a golden" in result["errors"][0]["error"]["message"]
        # Nothing was written: no partial/untrustworthy baseline on disk.
        assert not (project / "tests" / "goldens" / "nb.py" / "manifest.json").exists()

    def test_update_refusal_exit_code(self, project: pathlib.Path):
        path = write_nb(project)
        payload, exit_code = verify_tools.run_verify(
            [path], update=True, session_factory=self.factory(settled=False)
        )
        assert exit_code == 2, payload

    def test_update_accepts_settled_golden(self, project: pathlib.Path):
        """The settled path is unchanged: goldens record normally."""
        path = write_nb(project)
        result = verify_tools.update_target(
            path, session_factory=self.factory(settled=True)
        )
        assert result["status"] == "updated"
        manifest = json.loads(
            (project / "tests" / "goldens" / "nb.py" / "manifest.json").read_text()
        )
        shot = manifest["blocks"][0]["screenshot"]
        # A stored golden never carries a settled flag: it is settled by
        # construction, since an unsettled one is refused.
        assert "settled" not in shot

    def test_verify_warns_loudly_on_unsettled_comparison(self, project: pathlib.Path):
        """Verify reports the flag and warns; it does not invent a failure."""
        path = write_nb(project)
        verify_tools.update_target(path, session_factory=self.factory(settled=True))
        payload, exit_code = verify_tools.run_verify(
            [path], session_factory=self.factory(settled=False)
        )
        target_payload = payload["targets"][0]
        # The hashes still match (same fake png), so this is not a mismatch...
        assert exit_code == 0, target_payload
        block = target_payload["blocks"][0]
        assert block["pixels"]["match"] is True
        # ...but the result is explicitly flagged as not a reliable signal.
        assert block["pixels"]["settled"] is False
        assert "still animating" in block["pixels"]["settle_reason"]
        assert any(
            "not a reliable signal" in w for w in target_payload.get("warnings", [])
        )

    def test_verify_settled_comparison_is_unflagged(self, project: pathlib.Path):
        """A settled verify carries no settle keys and no warning."""
        path = write_nb(project)
        verify_tools.update_target(path, session_factory=self.factory(settled=True))
        payload, exit_code = verify_tools.run_verify(
            [path], session_factory=self.factory(settled=True)
        ...
        assert exit_code == 0
        block = target_payload["blocks"][0]
        assert "settled" not in block["pixels"]
        assert not any(
            "still animating" in w for w in target_payload.get("warnings", [])
        )
