...
from colight.scene3d import Cuboid, Ellipsoid, Group, Scene, deco
...
# =============================================================================
# Scene-level dict layers: unknown keys must not be silently ignored
# =============================================================================

# A dict layer becomes top-level scene props on the JS side (collectLayers ->
# SceneInner), which destructures only the props it knows and drops the rest.
# Scene() validates the keys so a dead prop is a loud error instead of a no-op.
...
def test_bare_camera_dict_raises_with_default_camera_hint():
    """The classic footgun: passing camera params where scene props go."""
    with pytest.raises(ValueError) as excinfo:
        Scene(Cuboid(center=[0, 0, 0], half_size=0.1), CAMERA_PARAMS)
    message = str(excinfo.value)
    assert "camera-shaped" in message
    assert "defaultCamera" in message


def test_bare_camera_dict_raises_when_added():
    """`component + camera_dict` routes through Scene() and must also raise."""
    with pytest.raises(ValueError, match="defaultCamera"):
        Cuboid(center=[0, 0, 0], half_size=0.1) + CAMERA_PARAMS


def test_unknown_scene_prop_raises():
    with pytest.raises(ValueError) as excinfo:
        Scene(Cuboid(center=[0, 0, 0], half_size=0.1), {"defaultCamer": CAMERA_PARAMS})
    message = str(excinfo.value)
    assert "defaultCamer" in message
    # The error lists the valid props so the typo is fixable from the message.
    assert "defaultCamera" in message


def test_valid_scene_props_accepted():
    scene = Scene(
        Cuboid(center=[0, 0, 0], half_size=0.1),
        {"defaultCamera": CAMERA_PARAMS, "controls": ["fps"], "height": 400},
    )
    props = scene.for_json()[1]
    # The dict layer survives to the JS boundary intact.
    layer = next(
        layer
        for layer in props["layers"]
        if isinstance(layer, dict) and "defaultCamera" in layer
    )
    assert layer["defaultCamera"]["fov"] == 40
    assert layer["controls"] == ["fps"]
