...
from colight_publish.static.builder import BuildConfig
from colight_publish.static.watcher import watch_build_and_serve
...
def test_watch_build_and_serve_creates_output_directory(tmp_path):
    ...
    with patch("colight_publish.static.watcher.LiveReloadServer") as mock_server:
        with patch("colight_publish.static.watcher.watch") as mock_watch:
            with patch("colight_publish.static.watcher.threading.Thread"):
...
def test_watch_build_and_serve_builds_files_for_directory(tmp_path):
    ...
    with patch("colight_publish.static.watcher.LiveReloadServer"):
        with patch("colight_publish.static.watcher.threading.Thread"):
...
def test_watch_build_and_serve_defaults_to_html_format(tmp_path):
    ...
    with patch("colight_publish.static.watcher.LiveReloadServer"):
        with patch("colight_publish.static.watcher.watch") as mock_watch:
            with patch("colight_publish.static.watcher.threading.Thread"):
                with patch(
                    "colight_publish.static.watcher.builder.build_file"
                ) as mock_build:
                    # Make watch raise KeyboardInterrupt to exit immediately
                    mock_watch.side_effect = KeyboardInterrupt()
