...
This specification describes the live sync behavior between the colight-publish server and client. The implementation is located in:

- **Client**: `packages/colight-publish/src/js/live.jsx` (main app), `websocket-message-handler.js` (message processing), `TopBar.jsx` (UI components)
- **Server**: `packages/colight-publish/src/colight_publish/server.py` (WebSocket server and file watching)
