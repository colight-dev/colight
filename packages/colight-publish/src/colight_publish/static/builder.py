...
from colight_publish.executor import DocumentExecutor
from colight_publish.file_resolver import find_files
from colight_publish.model import TagSet
from colight_publish.parser import parse_colight_file
from colight_publish.pep723 import detect_pep723_metadata, parse_dependencies
from colight_publish.pragma import parse_pragma_arg
from colight_publish.utils import merge_ignore_patterns
...
def build_file(
    ...
    **kwargs,
    ...
    if pep723_metadata and not config.in_subprocess:
        ...
        # Get the colight-publish package root (3 levels up from builder.py)
        colight_publish_root = pathlib.Path(__file__).parent.parent.parent
        cmd = [
            "uv",
            "run",
            "--with-editable",
            str(colight_publish_root),  # colight-publish package root
        ]
