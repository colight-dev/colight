...
**Key insight**: The serializer (`pack_message`) and type generator (`generate_typescript`) both use the same **handler registry**. Each handler knows how to serialize values *and* generate TypeScript types, ensuring they stay in sync.
...
function useWireSocket(url: string) {
  ...
  const pending = useRef<{ envelope: any; buffers: ArrayBuffer[] } | null>(null);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (pending.current!.buffers.length === pending.current!.envelope.buffer_count) {
